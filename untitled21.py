...
def variation(a_init, a_final):
    ...
    sep = a_final/a_init
    x_list = {0:[]}
    for a in range(a_int, a_final,sep):
...
a = 1.5
aend = 4
convergence = find_fix_point(a, aend)
...
def timestep():
    ...
    t_list= [x for x in range(0.0,t_max,0.1)]
    
    x = [0 for x in range(0, len(t_list))]
    y = [0 for x in range(0, len(t_list))]
    
    for i in range(1, len(t_list)):
        t = t_list[i]
        tchange= t - t_list[i-1]
        
        y = y * t - tchange*g
        #that initializes the
    
    #now solve for question
    #up speed 100ms-1
    #angle 30 deg
    #dt = 0.01 s
    ...
    #problem 3b
    #plot trajectory as x / y for the projectile until it reaches the ground
